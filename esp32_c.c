...
const uint32_t PULSE_TIMEOUT_US = 6000UL; // ~1 m max, very responsive
const int SAMPLES = 1;
const int SAMPLE_DELAY_MS = 0;
...
float readDistanceAveragedCM() {
  float sum = 0; int good = 0;
  for (int i=0;i<SAMPLES;i++){ float d=readDistanceOnceCM(); if(d>0){sum+=d;good++;} delay(SAMPLE_DELAY_MS); }
...
void setup() {
  ...
  Serial.println();
  Serial.begin(115200);           // keep your existing baud if different
pinMode(ULTRA_TRIG, OUTPUT);
pinMode(ULTRA_ECHO, INPUT);
digitalWrite(ULTRA_TRIG, LOW);  // idle low
...
void loop() {
float d_cm = readDistanceAveragedCM();
if (d_cm < 0) {
