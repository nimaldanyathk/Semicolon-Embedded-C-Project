...
// Continuous LiDAR reader (your logic kept)
void readLidarContinuously() {
  while (LidarSerial.available() >= 9) {
    int b1 = LidarSerial.read();
    if (b1 != 0x59) continue;
    int b2 = LidarSerial.read();
    if (b2 != 0x59) continue;

    uint8_t buf[7];
    for (int i = 0; i < 7; i++) {
      int r = LidarSerial.read();
      if (r < 0) return; // incomplete frame
      buf[i] = (uint8_t)r;
    }
...
static esp_err_t events_get_handler(httpd_req_t *req) {
  ...
  // flush quickly so client sees connection established
...
(function startSSE(){
  if (!('EventSource' in window)) {
    ...
    // ultra-light fallback polling (rarely used)
    (async function poll(){
      for(;;){
        try{
  ...
  es.onmessage = e=>{
    try {
      const j = JSON.parse(e.data); // {"lidar":..,"temp":..}
      updateUI(j.lidar, j.temp);
    } catch(_){}
  };
  es.onerror = e=>{
    // Network hiccup: EventSource auto-reconnects; just log.
...
void loop() {
  readLidarContinuously(); // continuously read LiDAR data
  delay(0);
}
